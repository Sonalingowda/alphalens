...
INTRADAY_PIPELINE_VERSION = "2.0.0"
    ...
    *,
    ...
    if asset_identifier != "BTC" or quote_currency != "USD":
        raise FeatureComputationError(
            "Phase 3 source snapshots support only BTC/USD."
        )
...
def _approved_definitions_by_identifier(
) -> dict[str, IntradayFeatureDefinition]:
    definitions = {
        definition.metadata.identifier: definition
        for definition in TIER_A_FEATURE_DEFINITIONS
    }
    if len(definitions) != len(TIER_A_FEATURE_DEFINITIONS):
        raise FeatureComputationError(
...
def _verify_prefix_invariance(
    ...
    for prefix_length in range(1, len(snapshot.observations) + 1):
        ...
        prefix_values = definition.compute(prefix, snapshot.timeframe)
        prefix_end = _required_timestamp(prefix[-1].timestamp)
        expected = tuple(
            value
            for value in full_values
            if value.timestamp <= prefix_end
        )
...
def _validate_pipeline_values(
    ...
    for value in values:
        ...
        if value.candle_timestamp not in source_timestamps:
            raise FeatureComputationError(
                "Pipeline emitted a non-source timestamp."
            )
...
def _validate_source_observations(
    ...
    for observation in observations:
        ...
        if timestamp.tzinfo is None or timestamp.utcoffset() is None:
            raise FeatureComputationError(
                "Source timestamp must be timezone-aware."
            )
        if timestamp.utcoffset() != timedelta(0):
            raise FeatureComputationError(
                "Source timestamp must be canonical UTC."
            )
        if floor_timeframe_boundary(timestamp, timeframe) != timestamp:
            raise FeatureComputationError(
                "Source timestamp is not timeframe-aligned."
            )
        if (
...
def _validate_source_values(candle: Candle) -> None:
    ...
    if any(
        ...
        raise FeatureComputationError(
            "Source observation contains a non-finite value."
        )
        ...
        "values": [
            {
                ...
                "candle_timestamp": _canonical_timestamp(
                    value.candle_timestamp
                ),
...
def _required_timestamp(value: datetime | None) -> datetime:
    if value is None:
        raise FeatureComputationError(
            "Source timestamp is unexpectedly missing."
        )
    return value
...
def _required_decimal(value: Decimal | None) -> Decimal:
    if value is None:
        raise FeatureComputationError(
            "Source value is unexpectedly missing."
        )
    return value
